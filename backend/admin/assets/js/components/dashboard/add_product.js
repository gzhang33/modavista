// htdocs/admin/assets/js/components/dashboard/add_product.js
...
export default class ProductFormComponent extends BaseComponent {
    ...
    populate_product_form(product) {
        ...
        // 设置表单字段
        if (product.id) {
            const idInput = this.form.querySelector('#product-id');
        ...
        if (this.category_select && product.category) {
            const italianCategory = this.getItalianNameFromMapping(this.categoryMapping, product.category) || product.category;
            this.category_select.value = italianCategory;
        }
        
        // 设置材质 - 使用映射关系将英文材质转换为意大利语
        if (this.material_select && product.material) {
            const italianMaterial = this.getItalianNameFromMapping(this.materialMapping, product.material) || product.material;
            this.material_select.value = italianMaterial;
        }
        
        // 设置季节 - 使用映射关系将英文季节转换为意大利语
        if (this.season_select && product.season) {
            const italianSeason = this.getItalianNameFromMapping(this.seasonMapping, product.season) || product.season;
            this.season_select.value = italianSeason;
        }
        
        // 设置颜色 - 使用映射关系将英文颜色转换为意大利语
        if (this.color_select && product.color) {
            const italianColor = this.getItalianNameFromMapping(this.colorMapping, product.color) || product.color;
            this.color_select.value = italianColor;
        }
        
        // 显示媒体预览
        if (Array.isArray(product.media) && product.media.length > 0) {
            this.show_current_media_previews(product.media, product.defaultImage || product.media[0]);
        }
    ...
    async show_form(product = null) {
        ...
        if (product && product.id && (!product.name && !product.base_name && !product.category)) {
            try {
                // 显示加载状态
                this.eventBus.emit('toast:show', { message: '正在加载产品数据...', type: 'info' });
                
                // 使用 apiClient 获取产品详情，修正为相对 API 路径并兼容返回格式
                const response = await apiClient.get('/products.php', { id: product.id, lang: 'it' });
                const productData = (response && response.success && response.data) ? response.data : response;
                if (productData && (productData.id || productData.product_id)) {
                    // 合并获取到的产品数据
                    product = { ...product, ...productData };
                    // 继续处理产品数据
                    return this.populate_product_form(product);
                } else {
                    throw new Error((response && response.message) || '获取产品详情失败');
                }
            } catch (e) {
    ...
    async handle_form_submit(e) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (!color) {
                ...
                this.is_saving = false; // Re-enable save button
        ...
        try {
            ...
            let error_message = '操作失败: ';
            if (error.response) {
                error.response.json().then(body => {
                    console.error('API Error Body:', body);
                    error_message += body.message || JSON.stringify(body);
                    this.eventBus.emit('toast:show', { message: error_message, type: 'error' });
                });
            } else {
