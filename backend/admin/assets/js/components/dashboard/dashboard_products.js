// htdocs/admin/assets/js/components/dashboard/dashboard_products.js - v1.1
import BaseComponent from '../shared/BaseComponent.js';
import apiClient from '../../utils/apiClient.js';
import { get_base_name, extract_color_label, color_name_to_hex } from '../../utils/product_name_utils.js';
...
export default class ProductTableComponent extends BaseComponent {
    constructor(selector, eventBus) {
        ...
        // Sorting state (default: relevance => keep original order from API)
        this.sortCriterion = 'relevance';
    ...
    init_listeners() {
        ...
        // Listen for sort changes dispatched by dashboard.php
        this.element.addEventListener('sortChanged', (e) => {
        ...
        list.forEach(p => {
            ...
            if (raw) {
                if (raw.startsWith('http')) {
                    ...
                    image_src = '/storage/uploads/product_images/' + raw.substring(9);
    ...
    get_current_list() {
        const base = (Array.isArray(this.filtered_products) && this.filtered_products !== null)
            ? this.filtered_products
            : this.all_products;
        return this.get_sorted_list(base);
    }

    get_sorted_list(list) {
        if (!Array.isArray(list)) return [];
        const criterion = this.sortCriterion || 'relevance';
        // relevance: keep original order
        if (criterion === 'relevance') return list.slice();
        const clone = list.slice();
        switch (criterion) {
            case 'newest':
                return clone.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
            case 'oldest':
                return clone.sort((a,b) => new Date(a.createdAt) - new Date(b.createdAt));
            case 'name_az':
                return clone.sort((a,b) => (a.base_name || a.name || '').localeCompare(b.base_name || b.name || ''));
            case 'name_za':
                return clone.sort((a,b) => (b.base_name || b.name || '').localeCompare(a.base_name || a.name || ''));
            default:
                return clone;
        }
    }
