...
<body>
    ...
    <script type="module">
        ...
        function buildGroup(title, key, options){
            ...
            options.forEach(opt => {
                ...
                if (selected.has(opt)) {
                    ...
                    btn.innerHTML = `${opt} <span class="chip-close" title="点击取消">×</span>`;
                } else {
                    btn.textContent = opt;
                }
                btn.addEventListener('click', () => {
                    const isActive = btn.classList.contains('active');
                    // 单选并支持取消选择：已选中则取消；未选中则设为仅此选中
                    if (isActive) {
                        btn.classList.remove('active');
                        btn.textContent = opt; // remove × when deselected
                    } else {
                        Array.from(box.querySelectorAll('.chip')).forEach(c => c.classList.remove('active'));
                        btn.classList.add('active');
                        // remove × from others
                        Array.from(box.querySelectorAll('.chip')).forEach(c => {
                            if (c !== btn) c.textContent = c.getAttribute('data-value');
                        });
                        // add × to selected
                        btn.innerHTML = `${opt} <span class="chip-close" title="点击取消">×</span>`;
                    }
        ...
        // 取消父级事件代理（每个 chip 自身监听）
        document.getElementById('apply-btn').onclick = () => {
            const values = {};
            document.querySelectorAll('.filters-options').forEach(box => {
                const key = box.getAttribute('data-key');
                const selected = Array.from(box.querySelectorAll('.chip.active')).map(n=>n.getAttribute('data-value'));
